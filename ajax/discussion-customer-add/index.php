...
if(strpos($_SERVER['HTTP_REFERER'], COMBAT_DOMAIN) != false) {
    ...
    if(empty($arrParam[5]) && empty($arrParam[6])){
        ...
        echo $success;
    }elseif (!empty($arrParam[5]) && !empty($arrParam[6]) && !empty($_FILES['upfiles']['name'][0])){
        $callback = call_user_func_array($methodAction, $arrParam);
        // Если сообщение с генерацией временной ссылки
        $success = json_encode(['reqId' => $callback, 'countFiles' => count($_FILES['upfiles']['name']), 'createLink' => 1]);
        echo $success;
    }elseif (!empty($arrParam[5]) && !empty($arrParam[6]) && empty($_FILES['upfiles']['name'][0])){
        // $callback = call_user_func_array($methodAction, $arrParam);
